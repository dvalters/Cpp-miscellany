...
// We are writing 3D data, a 6 x 12 lat-lon grid, with 2
// timesteps of data.
#define NDIMS    3
//#define NLVL     0
#define NLAT     6
...
int main()
{
   ...
   for (int timestep = 0; timestep < NREC; timestep++)
     for (int lat = 0; lat < NLAT; lat++)
       for (int lon = 0; lon < NLON; lon++)
	 {
     pres_out[timestep][lat][lon] = (float) (SAMPLE_PRESSURE + i);
   ...
   try
   {
      ...
      NcDim recDim = test.addDim(REC_NAME, NREC);  //adds a NREC dimensions
